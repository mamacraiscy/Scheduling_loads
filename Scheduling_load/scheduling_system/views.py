...
def filter_programs(request):
    program_code = request.GET.get("program_code", "").split(",")
    shift = request.GET.get("shift", "")
    year_levels = request.GET.get("year_levels", "").split(",")

    programs = ProgramSchedule.objects.all()

    if program_code and program_code[0] != "":
        programs = programs.filter(program_code__in=program_code)

    if shift:
        programs = programs.filter(shift=shift)

    if year_levels and year_levels[0] != "":
        programs = programs.filter(year_level__in=year_levels)

    data = list(programs.values(
        "program_code", "program_name", "instructor_name",
        "room_number", "course_name", "year_level", "shift"
    ))
